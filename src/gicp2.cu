...
void convert(std::vector<int>& eu, std::vector<std::vector<long double>>& transform)
{
	float x=float(eu[0]), y=float(eu[1]), z=float(eu[2]), thetax=float(eu[3]), thetay=float(eu[4]), thetaz=float(eu[5]);
...
int main(int argc, char** argv)
{
	if(argc!=9)
	{
		std::cout<<"usage: ./gicp2 file_name1 file_name2 x y z thetax thetay thetaz\n";
	}
	else
	{
...
//		plot_merged(reading, reference);
		...
		int x=std::stoi(argv[3]), y=std::stoi(argv[4]), z=std::stoi(argv[5]), thetax=std::stoi(argv[6]), thetay=std::stoi(argv[7]), thetaz=std::stoi(argv[8]);
		std::vector<std::vector<long double>> t(4, std::vector<long double>(4, 0.0));
		std::vector<int> eu(6); eu[0]=x; eu[1]=y; eu[2]=z; eu[3]=thetax; eu[4]=thetay; eu[5]=thetaz;
		convert(eu, t);
		process(reading, t);		
...
//		plot_merged(reading, reference);
		...
		output_name.push_back('_'); output_name.append(std::to_string(x));
		output_name.push_back('_'); output_name.append(std::to_string(y));
		output_name.push_back('_'); output_name.append(std::to_string(z));
		output_name.push_back('_'); output_name.append(std::to_string(thetax));
		output_name.push_back('_'); output_name.append(std::to_string(thetay));
		output_name.push_back('_'); output_name.append(std::to_string(thetaz));
...
//		file_write(output_name, transformation_mat);
		...
		*/
		...
		std::cout<<map<<" "<<s1<<" "<<s2<<" "<<x<<" "<<y<<" "<<z<<" "<<thetax<<" "<<thetay<<" "<<thetaz<<" "<<ans[0]<<" "<<ans[1]<<" "<<ans[2]<<" "<<ans[3]<<" "<<ans[4]<<" "<<ans[5]<<std::endl;
//		plot_merged(reading, reference);
	}
	return 0;
}
